...
16. 명령어 이름은 함수의 작명규칙을 따른다. 

### 명령어
17. 사용처가 비슷한 함수, 변수끼리 같은 클래스로 만든다. \
  다른 클래스의 것과 사용 방법이 비슷한 함수, 변수끼리는 이름을 같게 한다. 

### 스타일
18. f"{변수명}" 형식을 표준으로 한다. 
19. 상태를 나타내거나 Literal을 사용하여 특정 값을 인수로 받을 때, 또는 그런 값을 리턴할 때는 작은따옴표를 쓰고, 나머지 경우에는 큰따옴표를 쓴다. 
20. 키워드 인수나 초기값을 설정하는 부분에서는 등호 양쪽에 띄어쓰기를 하지 않는다. 
21. 타입 힌트, 타입 어노테이션은 적는 것을 기본으로 한다. 
...
### 프로그램의 정체성에 관한 문서
23. 기존 문서를 삭제하고 현재 버전까지 다시 작성한다. 

### 메모 예시
24. 남아있던 메모 예시는 현재 버전까지 삭제한다. 

## 버전 작명 방식
25. (메이저 버전).(마이너 버전).(날짜코드) 형식으로 적는다.\
  불가피한 상황이 아니라면 날짜코드는 YYMN으로 한다. 단, N은 그 달에 나온 정식 버전 개수이다. (예시 : 2471은 2024년 7월의 첫 버전이다. )\
  10월, 11월, 12월은 각각 A, B, C로 표기한다. 
26. 현재까지 베타버전으로 계획된 버전을 정식 버전으로 승격한다. 
27. 베타버전은 같은 달 정식 버전이 공개된다고 가정한 날짜코드에서 1을 뺀 후 버전코드와 개발번호를 붙인다. (예시 : 3.0.2471의 베타 2는 3.0.2470 beta 2로 이름짓는다. )\
